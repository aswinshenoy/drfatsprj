...
const JobCard = ({ job }: { job: JobListType }) => {
    ...
    <Link href={`/j/${job.jobID}`} className="group">
      <div
        className={clsx([
          ...
          "border border-neutral-400/20 rounded-lg md:rounded-xl shadow hover:shadow-none transition-all duration-100",
        ])}
      >
        ...
        <div className="w-full md:w-1/3 lg:w-1/4 flex md:justify-end p-2">
          <Link href={`/j/${job.jobID}/apply`}>
            <Button>
              Apply
